...
public class GS1EpcTagDataTranslationEngine {
    ...
    /**
     * During the translation process, {@code parameterMap} will be populated with additional
    ...
    public String translate(String epcIdentifier, String parameterList, String outputFormat)
        ...
        parameterMap = util.parseParameterList(parameterList);

        LOG.info("outbound level is {}", LevelTypeList.fromValue(outputFormat));
        LOG.debug("2. DETERMINE THE CODING SCHEME AND INBOUND REPRESENTATION LEVEL.");

        Map<Level, Scheme> inputLevelsSchemes = getLevelSchemeMap(epcIdentifier);
        ...
        parseInputValue(epcIdentifier, inputLevel, inputScheme, inputOption);
        ...
        for (Level level : inputScheme.getLevel()) {
            if (level.getType() == LevelTypeList.fromValue(outputFormat)) {
                outputLevel = level;
                for (Option option : level.getOption()) {
     ...
     */
    private void performRules(Level outputLevel) throws TDTTranslationException {
        if (outputLevel != null) {
    ...
    private void checkOutput(Level outputLevel, Option outputOption) throws TDTTranslationException {
     ...
     * quotation marks.
     */
    private StringBuilder buildOutputValue(Scheme inputScheme, Option inputOption,
                                           Level outputLevel, Option outputOption) throws TDTTranslationException {
     ...
     */
    private Map<Level, Scheme> getLevelSchemeMap(String epcIdentifier) throws TDTTranslationException {
